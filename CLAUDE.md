...
## 배포 없이 페이지를 고쳐야 할 때 (핫픽스)

Docusaurus 페이지는 정적 HTML 로 먼저 그려진 뒤 React 가 JS 번들 데이터로 다시 그린다(hydration). 그래서 HTML 만 고치면 화면이 잠깐 바뀌었다가 원래대로 돌아온다. 핫픽스는 **HTML 과 해당 JS 청크를 같이** 고쳐야 하고, 두 결과가 같은 DOM 이 되도록 맞춰야 한다.

- 페이지 데이터는 `assets/js/<해시>.js` 안에 `JSON.parse("...")` 문자열로 들어 있고, 한글은 `\uXXXX` 로 이스케이프되어 있어 한글 그대로 grep 하면 안 잡힌다. 이스케이프한 문자열로 찾는다.
- 어떤 청크가 어떤 경로를 담당하는지는 `assets/js/main.<해시>.js` 의 라우트 표(`path:"/gd-dev-prototype/..."`)에서 확인한다.
- 소스 diff 페이지 컴포넌트는 `assets/js/d60551f1.73ae26fa.js` 다. 페이지 데이터의 `skinPatch` 값 하나가 상단 "스킨 패치" 칩, 스킨 직접 수정 안내 블록, "전체 / 스킨 파일" 탭을 모두 켠다. 2026-09-14 에 안내 블록만 빼기 위해 데이터 대신 이 컴포넌트에서 안내 요소 렌더링을 제거했고, `source-diff-826/15615.html` 에서도 같은 블록을 걷어냈다.
- 파일명의 해시는 바꾸지 않으므로 푸시 직후에는 브라우저·CDN 캐시 때문에 이전 청크가 잠시 보일 수 있다.
